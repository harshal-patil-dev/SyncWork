...
<link rel="stylesheet" type="text/css"
	href="${pageContext.request.contextPath}/css/update-employee.css">
</head>
<body>
	<c:set var="activePage" value="update" />
	...
	<main class="update-section">
		<div class="page-header">
			<div>
				...
				<h1>
					Update <span>Employee</span>
				</h1>
				<p>Find an employee by ID and update their workforce
					information.</p>
			</div>
		</div>
						...
						<div class="form-grid">
							...
							<div class="form-group">
								<label for="department">Department</label> <select
									...
									<option value="IT"
										${employee.department eq 'IT' ? 'selected' : ''}>IT</option>
									<option value="HR"
										${employee.department eq 'HR' ? 'selected' : ''}>HR</option>
									<option value="Testing"
										${employee.department eq 'Testing' ? 'selected' : ''}>Testing</option>
									<option value="Marketing"
										${employee.department eq 'Marketing' ? 'selected' : ''}>Marketing</option>
									<option value="Sales"
										${employee.department eq 'Sales' ? 'selected' : ''}>Sales</option>
								</select>
							</div>
							<div class="form-group">
								<label for="salary">Salary</label> <input type="number"
									id="salary" name="salary" value="${employee.salary}" min="0"
									required>
							</div>
							<div class="form-group">
								<label for="experience">Experience</label> <input type="number"
									id="experience" name="experience"
									value="${employee.experience}" min="0" required>
							</div>
							<div class="form-group full-width">
								<label for="status">Employee Status</label> <select id="status"
									name="status" required>
									<option value="Active"
										${employee.status eq 'Active' ? 'selected' : ''}>Active</option>
									<option value="Inactive"
										${employee.status eq 'Inactive' ? 'selected' : ''}>Inactive</option>
									<option value="On Leave"
										${employee.status eq 'On Leave' ? 'selected' : ''}>On
										Leave</option>
								</select>
							</div>
						</div>
